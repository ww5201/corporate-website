...
test_script = '''const fs=require('fs');
const js=fs.readFileSync('/tmp/fixed_js.txt','utf8');
class MockEl{
  constructor(t){this.tag=t;this.style={};this.innerHTML='';this.textContent='';
...
globalThis.document={
  getElementById:(id)=>{const e=new MockEl(id);e.id=id;return e;},
...
stdin, stdout, stderr = ssh.exec_command("cat > /tmp/test2.js << 'ENDSCRIPT'\n" + test_script + "\nENDSCRIPT")
stderr.read().decode()

stdin, stdout, stderr = ssh.exec_command('node /tmp/test2.js 2>&1')
result = stdout.read().decode('utf-8', errors='replace')
err = stderr.read().decode('utf-8', errors='replace')

with open('D:/tokai/node_result2.txt', 'w', encoding='utf-8') as f:
    f.write(f"STDOUT:\n{result}\nSTDERR:\n{err}")
