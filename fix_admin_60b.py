...
ssh.exec_command("sed -i 's/imgUrls.length + files.length > 20/imgUrls.length + files.length > 60/g' /root/backend/admin.html")
ssh.exec_command("sed -i 's/最多只能上传20张图片/最多只能上传60张图片/g' /root/backend/admin.html")
...
# 用文件记录验证结果
stdin, stdout, stderr = ssh.exec_command("grep -n '60' /root/backend/admin.html | grep -i 'files\\|length\\|upload'")
result = stdout.read().decode('utf-8', errors='replace')
with open(r'D:\tokai\admin_verify.txt', 'w', encoding='utf-8') as f:
    f.write(result)
