...
stdin, stdout, stderr = ssh.exec_command("pm2 stop all && pm2 delete all && echo 'pm2 cleaned'")
pm2_clean = stdout.read().decode('utf-8', errors='replace').strip()
print(f"PM2: {pm2_clean}")
...
# 2. 检查端口是否释放
stdin, stdout, stderr = ssh.exec_command("lsof -i :3000 -i :3001")
...
# 7. 最终进程列表
stdin, stdout, stderr = ssh.exec_command("ps aux | grep 'node\\|next\\|nginx' | grep -v grep")
