...
const WECHAT = {
  appId: process.env.WECHAT_APP_ID || 'wxbf90ff43ddc3b955',
  appSecret: process.env.WECHAT_APP_SECRET || 'c6c792b88f9020f1cab0b82b6b34ceaf',
};
...
// ========== WeChat Login (Mobile App OAuth2) ==========
// Flow: APP native SDK -> SendAuth.Req -> code -> backend exchanges for user info
router.post('/wechat/login', async (req, res) => {
  ...
  try {
    // Step 1: Exchange code for access_token + openid (Mobile App OAuth2)
    ...
    console.log('[WeChat Login] Token response:', JSON.stringify(tokenRes));

    if (tokenRes.errcode && tokenRes.errcode !== 0) {
      console.error('[WeChat Login] Token error:', tokenRes.errcode, tokenRes.errmsg);
      return res.status(400).json({
        error: `\u5fae\u4fe1\u6388\u6743\u5931\u8d25: ${tokenRes.errmsg || '\u672a\u77e5\u9519\u8bef'}`,
    ...
    const unionid = tokenRes.unionid || null;

    if (!openid || !accessToken) {
      return res.status(400).json({ error: '\u83b7\u53d6\u5fae\u4fe1\u6388\u6743\u5931\u8d25' });
    }

    // Step 2: Get user info (nickname, avatar) using access_token
    let wxNickname = nickname || '';
    let wxAvatar = avatar || '';
    try {
      ...
      if (infoRes.nickname) {
        wxNickname = infoRes.nickname;
        wxAvatar = infoRes.headimgurl || wxAvatar;
      }
    } catch (e) {
      console.warn('[WeChat Login] Failed to get user info:', e.message);
    }
    ...
    userDB.findOne({ wechatOpenId: openid }, (err, user) => {
      ...
      if (!user) {
        ...
        const newUser = {
          wechatOpenId: openid,
          unionId: unionid,
          nickname: wxNickname || nickname || '\u5fae\u4fe1\u7528\u6237',
          avatar: wxAvatar || avatar || '',
          role: 'user',
          lastLogin: new Date().toISOString(),
          createdAt: new Date().toISOString(),
        };
